...
function LivescoreAPI (){
    ...
    this.verify = (callback) => {
        request(this.buildUrl("users/pair.json"), {
            json: true
        }, (err, res, body) => {
            if (err) {
                return callback(err);
    ...
    this.getAllFederations = (callback) => {
        request(this.buildUrl("federations/list.json"), {
            json: true
        }, (err, res, body) => {
            if (err) {
                return callback(err);
    ...
    /* get fixtures with filters
       ...
       team => integer(team))
       round => string (round) */
    this.getFixtursFiltered = (filters, callback) => {
        let curFilter = [];
        ...
        if (filters.competition_id != undefined) {
            ...
            if (checkCompetition == 0) {
                return callback(new Error("Incorrect parameter! The competition id must be an integer!"));
            } else if (checkCompetition == -1) {
                return callback(new Error("Incorrect parameter! The competition id must be bigger than 0!"));
            } else if (checkCompetition == 1) {
                curFilter.push({
                    name: 'competition_id',
        ...
        if (filters.team != undefined) {
            ...
            if (checkTeam == 0) {
                return callback(new Error("Incorrect parameter! The team id must be an integer!"));
            } else if (checkTeam == -1) {
                return callback(new Error("Incorrect parameter! The team id be bigger than 0!"));
            } else if (checkTeam == 1) {
                curFilter.push({
                    name: 'team',
                    value: filters.team
        ...
        if (curFilter.length >= 1) {
            request(this.buildUrl('fixtures/matches.json', curFilter), {
                json: true
            }, (err, res, body) => {
                if (err) {
                    return callback(err);
    ...
    this.getFixturesDateLanguageCompetition = (date, language, competition_id, callback) => {
    ...
    this.getFixturesCompetitionLanguage = (competition_id, language, callback) => {
        checkLanguage = this.checkLanguage(language);
        checkCompetition = this.checkCouComPagTeaFed(competition_id);
        if (checkLanguage == 0) {
            return callback(new Error("Incorrect parameter! The language must be a string!"));
    ...
    this.getHistoryBetweendDatesCountryLanguage = (fromDate, toDate, competition_id, language, callback) => {
        ...
        if (checkLanguage == 1 && checkCompetition == 1 && checkFromDate == 1 && checkToDate == 1) {
            request(this.buildUrl('scores/history.json', [{
                name: 'from',
                value: fromDate
            }, {
    ...
    this.getHistoryBetweenDatesCompetition = (fromDate, toDate, competition_id, callback) => {
        checkFromDate = this.checkDate(fromDate);
        checkToDate = this.checkDate(toDate);
        checkCompetition = this.checkCouComPagTeaFed(competition_id);
        if (checkFromDate == 0 || checkFromDate == -1 || checkToDate == 0 || checkToDate == -1) {
    ...
    this.getHistoryFromDateCompetitionLanguage = (fromDate, competition_id, language, callback) => {
        checkFromDate = this.checkDate(fromDate);
        checkCompetition = this.checkCouComPagTeaFed(competition_id);
        checkLanguage = this.checkLanguage(language);
    ...
    this.getHistoryToDateCompetitionLanguage = (toDate, competition_id, language, callback) => {
        checkToDate = this.checkDate(toDate);
        checkCompetition = this.checkCouComPagTeaFed(competition_id);
        checkLanguage = this.checkLanguage(language);
    ...
    this.getAllCompetitions = (callback) => {
        request(this.buildUrl('competitions/list.json'), {
            json: true
        }, (err, res, body) => {
            if (err) {
                return callback(err);
    ...
    //get competitions with fixtures
    this.getCompetitionsWithFixtures = (callback) => {
        request(this.buildUrl('fixtures/competitions.json'), {
            json: true
        }, (err, res, body) => {
            if (err) {
                return callback(err);
            ...
            callback(null, res.body.data.competitions);
        })
    }
    ...
    this.getCompetitionsFromFederation = (federation_id, callback) => {
        ...
        if (check == 0) {
            return callback(new Error("Incorrect parameter! The federation id must be an integer!"));
        } else if (check == -1) {
            return callback(new Error("Incorrect parameter! The federation id must be bigger than 0!"));
        } else if (check == 1) {
            request(this.buildUrl("competitions/list.json", [{
                name: 'federation_id',
                value: federation_id
    ...
    this.getLiveScoresCountryCompetitionLanguage = (country_id, competition_id, language, callback) => {
    ...
    this.getLiveScoresCountryCompetiton = (country_id, competition_id, callback) => {
        checkCountry = this.checkCouComPagTeaFed(country_id);
        checkCompetition = this.checkCouComPagTeaFed(competition_id);
        if (checkCountry == 0) {
            return callback(new Error("Incorrect parameter! The country id must be an integer!"));
    ...
    this.getLiveScoresCompetitionLanguage = (competition_id, language, callback) => {
        checkCompetition = this.checkCouComPagTeaFed(competition_id);
        checkLanguage = this.checkLanguage(language);
        if (checkLanguage == 0) {
            return callback(new Error("Incorrect parameter! The language must be a string!"));
